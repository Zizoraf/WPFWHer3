...
public static class DbSetMock
{
    public static DbSet<T> Create<T>(IQueryable<T> data) where T : class
    {
        ...
        mockSet.As<IAsyncEnumerable<T>>().Setup(m => m.GetAsyncEnumerator(It.IsAny<System.Threading.CancellationToken>())).Returns(new AsyncEnumerator<T>(data.GetEnumerator()));
